//! WebGPU tensor-level smoke tests through numr's public API.
//!
//! Run: cargo test --features wgpu --test wgpu_tensor_ops

#![cfg(feature = "wgpu")]

use numr::ops::{ActivationOps, BinaryOps, CompareOps, MatmulOps, ReduceOps, ScalarOps};
use numr::runtime::Runtime;
use numr::runtime::wgpu::{WgpuDevice, WgpuRuntime, is_wgpu_available};
use numr::tensor::Tensor;

fn create_test_tensor(data: &[f32], shape: &[usize]) -> Tensor<WgpuRuntime> {
    let device = WgpuDevice::new(0);
    Tensor::<WgpuRuntime>::from_slice(data, shape, &device).unwrap()
}
