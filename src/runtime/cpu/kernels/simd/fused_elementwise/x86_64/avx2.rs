//! AVX2+FMA fused elementwise kernels (256-bit)
...
/// AVX2+FMA fused_mul_add for f32: out = a * b + c
...
pub unsafe fn fused_mul_add_f32(
    ...
    for i in 0..chunks {
        ...
        let vc = _mm256_loadu_ps(c.add(offset));
        // FMA: va * vb + vc in single instruction
...
/// AVX2+FMA fused_mul_add for f64: out = a * b + c
#[target_feature(enable = "avx2", enable = "fma")]
pub unsafe fn fused_mul_add_f64(
    ...
    for i in 0..chunks {
        ...
        let vc = _mm256_loadu_pd(c.add(offset));
...
/// AVX2 fused_add_mul for f32: out = (a + b) * c
#[target_feature(enable = "avx2", enable = "fma")]
pub unsafe fn fused_add_mul_f32(
    ...
    for i in 0..chunks {
        ...
        let result = _mm256_mul_ps(sum, vc);
        _mm256_storeu_ps(out.add(offset), result);
    }
...
/// AVX2 fused_add_mul for f64: out = (a + b) * c
#[target_feature(enable = "avx2", enable = "fma")]
pub unsafe fn fused_add_mul_f64(
    ...
    for i in 0..chunks {
        ...
        let result = _mm256_mul_pd(sum, vc);
        _mm256_storeu_pd(out.add(offset), result);
    }
...
/// AVX2+FMA fused_mul_add_scalar for f32: out = a * scale + bias
#[target_feature(enable = "avx2", enable = "fma")]
pub unsafe fn fused_mul_add_scalar_f32(
    ...
    for i in 0..chunks {
        let offset = i * F32_LANES;
        let va = _mm256_loadu_ps(a.add(offset));
        let result = _mm256_fmadd_ps(va, vscale, vbias);
        _mm256_storeu_ps(out.add(offset), result);
    }
...
/// AVX2+FMA fused_mul_add_scalar for f64: out = a * scale + bias
#[target_feature(enable = "avx2", enable = "fma")]
pub unsafe fn fused_mul_add_scalar_f64(
    ...
    let vbias = _mm256_set1_pd(bias);

    for i in 0..chunks {
        let offset = i * F64_LANES;
        let va = _mm256_loadu_pd(a.add(offset));
        let result = _mm256_fmadd_pd(va, vscale, vbias);
        _mm256_storeu_pd(out.add(offset), result);
    }
