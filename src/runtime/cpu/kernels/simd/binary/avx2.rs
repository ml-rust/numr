...
pub unsafe fn binary_f32(op: BinaryOp, a: *const f32, b: *const f32, out: *mut f32, len: usize) {
    ...
    match op {
        ...
        BinaryOp::Pow => binary_pow_f32_avx2(a, b, out, chunks),
    }
...
pub unsafe fn binary_f64(op: BinaryOp, a: *const f64, b: *const f64, out: *mut f64, len: usize) {
    ...
    match op {
        ...
        BinaryOp::Pow => binary_pow_f64_avx2(a, b, out, chunks),
    }
...
unsafe fn binary_pow_f32_avx2(a: *const f32, b: *const f32, out: *mut f32, chunks: usize) {
    for i in 0..chunks {
        let offset = i * F32_LANES;
        let va = _mm256_loadu_ps(a.add(offset));
        let vb = _mm256_loadu_ps(b.add(offset));
...
///
/// See `binary_pow_f32_avx2` for algorithm and edge case documentation.
///
/// # Accuracy
/// Relative error < 1e-4 due to compound error from exp(b*log(a))
#[target_feature(enable = "avx2", enable = "fma")]
unsafe fn binary_pow_f64_avx2(a: *const f64, b: *const f64, out: *mut f64, chunks: usize) {
    for i in 0..chunks {
        let offset = i * F64_LANES;
        let va = _mm256_loadu_pd(a.add(offset));
        let vb = _mm256_loadu_pd(b.add(offset));
