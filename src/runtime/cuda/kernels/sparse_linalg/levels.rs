...
// ============================================================================
// LU Split Structure Analysis
// ============================================================================

/// Classify each matrix entry as L or U
pub unsafe fn launch_classify_lu_entries(
    context: &Arc<CudaContext>,
    stream: &CudaStream,
    device_index: usize,
    row_ptrs: u64,
    ...
    let module = get_or_load_module(context, device_index, SPARSE_LEVELS_MODULE)?;
    let func = get_kernel_function(&module, "classify_lu_entries")?;
    let cfg = launch_config((grid_size(n as u32), 1, 1), (BLOCK_SIZE, 1, 1), 0);
    ...
    builder.arg(&u_flags);
    builder.arg(&n);
    unsafe { builder.launch(cfg) }.map_err(|e| launch_error("classify_lu_entries", e))?;
    Ok(())
}

/// Count L and U entries per row
pub unsafe fn launch_count_lu_per_row(
    context: &Arc<CudaContext>,
    stream: &CudaStream,
    device_index: usize,
    row_ptrs: u64,
    ...
    let module = get_or_load_module(context, device_index, SPARSE_LEVELS_MODULE)?;
    let func = get_kernel_function(&module, "count_lu_per_row")?;
    let cfg = launch_config((grid_size(n as u32), 1, 1), (BLOCK_SIZE, 1, 1), 0);

    let mut builder = stream.launch_builder(&func);
    builder.arg(&row_ptrs);
    builder.arg(&col_indices);
    builder.arg(&l_counts);
    builder.arg(&u_counts);
    builder.arg(&n);
    unsafe { builder.launch(cfg) }.map_err(|e| launch_error("count_lu_per_row", e))?;
    Ok(())
}

/// Build mapping arrays for LU split
pub unsafe fn launch_build_lu_maps(
    context: &Arc<CudaContext>,
    stream: &CudaStream,
    device_index: usize,
    row_ptrs: u64,
    ...
    let module = get_or_load_module(context, device_index, SPARSE_LEVELS_MODULE)?;
    let func = get_kernel_function(&module, "build_lu_maps")?;
    let cfg = launch_config((grid_size(n as u32), 1, 1), (BLOCK_SIZE, 1, 1), 0);

    let mut builder = stream.launch_builder(&func);
    builder.arg(&row_ptrs);
    builder.arg(&col_indices);
    builder.arg(&l_row_ptrs);
    builder.arg(&u_row_ptrs);
    builder.arg(&l_col_indices);
    builder.arg(&u_col_indices);
    builder.arg(&l_map);
    builder.arg(&u_map);
    builder.arg(&n);
    unsafe { builder.launch(cfg) }.map_err(|e| launch_error("build_lu_maps", e))?;
    Ok(())
}

// ============================================================================
// Lower Triangle Extraction (IC)
// ============================================================================

/// Count lower triangle entries per row
pub unsafe fn launch_count_lower_per_row(
    context: &Arc<CudaContext>,
    stream: &CudaStream,
    device_index: usize,
    row_ptrs: u64,
    ...
    let module = get_or_load_module(context, device_index, SPARSE_LEVELS_MODULE)?;
    let func = get_kernel_function(&module, "count_lower_per_row")?;
    let cfg = launch_config((grid_size(n as u32), 1, 1), (BLOCK_SIZE, 1, 1), 0);

    let mut builder = stream.launch_builder(&func);
    builder.arg(&row_ptrs);
    builder.arg(&col_indices);
    builder.arg(&lower_counts);
    builder.arg(&n);
    unsafe { builder.launch(cfg) }.map_err(|e| launch_error("count_lower_per_row", e))?;
    Ok(())
}

/// Build mapping for lower triangle extraction
pub unsafe fn launch_build_lower_map(
    context: &Arc<CudaContext>,
    stream: &CudaStream,
    device_index: usize,
    row_ptrs: u64,
    col_indices: u64,
    lower_row_ptrs: u64,
    lower_col_indices: u64,
    lower_map: u64,
    n: i32,
) -> Result<()> {
    let module = get_or_load_module(context, device_index, SPARSE_LEVELS_MODULE)?;
    let func = get_kernel_function(&module, "build_lower_map")?;
    let cfg = launch_config((grid_size(n as u32), 1, 1), (BLOCK_SIZE, 1, 1), 0);

    let mut builder = stream.launch_builder(&func);
    builder.arg(&row_ptrs);
    builder.arg(&col_indices);
    builder.arg(&lower_row_ptrs);
    builder.arg(&lower_col_indices);
    builder.arg(&lower_map);
    builder.arg(&n);
    unsafe { builder.launch(cfg) }.map_err(|e| launch_error("build_lower_map", e))?;
    Ok(())
}
