// U32 broadcast binary operations
...
@group(0) @binding(0) var<storage, read_write> broadcast_a: array<u32>;
@group(0) @binding(1) var<storage, read_write> broadcast_b: array<u32>;
@group(0) @binding(2) var<storage, read_write> broadcast_out: array<u32>;
...
fn broadcast_add_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
...
@compute @workgroup_size(256)
fn broadcast_sub_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= broadcast_params.numel) { return; }
    var remaining = idx;
    var a_offset: u32 = 0u;
...
@compute @workgroup_size(256)
fn broadcast_mul_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= broadcast_params.numel) { return; }
    var remaining = idx;
    var a_offset: u32 = 0u;
...
@compute @workgroup_size(256)
fn broadcast_div_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= broadcast_params.numel) { return; }
    var remaining = idx;
    var a_offset: u32 = 0u;
...
@compute @workgroup_size(256)
fn broadcast_max_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= broadcast_params.numel) { return; }
    var remaining = idx;
    var a_offset: u32 = 0u;
...
@compute @workgroup_size(256)
fn broadcast_min_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if (idx >= broadcast_params.numel) { return; }
    var remaining = idx;
    var a_offset: u32 = 0u;
