...
/// Generate WGSL shader for slice_assign operation.
///
/// Copies src elements into the correct slice of the output tensor along a dimension.
/// Output should be pre-initialized with a copy of dst. This kernel overwrites the slice.
///
/// One thread per src element. Writes to:
///   output[outer * dst_dim_size * inner + (start + src_dim_idx) * inner + inner_idx]
pub fn generate_slice_assign_shader(dtype: DType) -> Result<String> {
    let t = wgsl_type(dtype)?;
    let suffix = dtype_suffix(dtype)?;

    Ok(format!(
        r#"// Auto-generated slice_assign operations for {t}
...
@compute @workgroup_size(256)
fn slice_assign_{suffix}(@builtin(global_invocation_id) gid: vec3<u32>) {{
    let idx = gid.x;
...
pub fn generate_gather_2d_shader(dtype: DType) -> Result<String> {
    ...
    Ok(format!(
        r#"// Auto-generated gather_2d operation for {t}
...
@compute @workgroup_size(256)
fn gather_2d_{suffix}(@builtin(global_invocation_id) gid: vec3<u32>) {{
    let idx = gid.x;
    if (idx >= params.num_indices) {{
        return;
    }}
