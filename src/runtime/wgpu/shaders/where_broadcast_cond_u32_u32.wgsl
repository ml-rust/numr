// where_broadcast_cond: condition=u32, output=u32
...
@group(0) @binding(1) var<storage, read_write> bc_x: array<u32>;
@group(0) @binding(2) var<storage, read_write> bc_y: array<u32>;
@group(0) @binding(3) var<storage, read_write> bc_out: array<u32>;
...
fn where_broadcast_cond_u32_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
