// Auto-generated sort operations for i32

const WORKGROUP_SIZE: u32 = 256u;
const MAX_SORT_SIZE: u32 = 512u;

var<workgroup> shared_vals: array<i32, 512>;
...
@group(0) @binding(0) var<storage, read_write> sort_input: array<i32>;
...
// Comparison helper
fn compare_less_i32(a: i32, b: i32) -> bool {
    return a < b;
}
...
fn bitonic_cas_values_i32(i: u32, j: u32, dir: bool) {
    let vi = shared_vals[i];
    let vj = shared_vals[j];
    ...
    for (var k: u32 = 2u; k <= n; k = k << 1u) {
        for (var j: u32 = k >> 1u; j > 0u; j = j >> 1u) {
            for (var i = tid; i < n / 2u; i = i + WORKGROUP_SIZE) {
                ...
                if (ij_pair < n) {
                    bitonic_cas_i32(ij, ij_pair, ascending_local);
                }
            }
            workgroupBarrier();
        }
    ...
    for (var k: u32 = 2u; k <= n; k = k << 1u) {
        for (var j: u32 = k >> 1u; j > 0u; j = j >> 1u) {
            for (var i = tid; i < n / 2u; i = i + WORKGROUP_SIZE) {
                ...
                if (ij_pair < n) {
                    bitonic_cas_values_i32(ij, ij_pair, ascending_local);
                }
            }
            workgroupBarrier();
        }
