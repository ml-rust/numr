// Semiring matmul: plus_max for f32
// C[i,j] = sum_k( max(A[i,k], B[k,j]) )
// Entry points: semiring_matmul_plus_max_f32, batched_semiring_matmul_plus_max_f32
...
fn semiring_matmul_plus_max_f32(
...
@compute @workgroup_size(16, 16, 1)
fn batched_semiring_matmul_plus_max_f32(
    @builtin(global_invocation_id) global_id: vec3<u32>
) {
    let M = sr_params.M;
    let K = sr_params.K;
