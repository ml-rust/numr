// where_broadcast_cond: condition=i32, output=u32
...
@group(0) @binding(0) var<storage, read_write> bc_cond: array<i32>;
...
fn where_broadcast_cond_i32_u32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    ...
    let cond_val = bc_cond[cond_offset] != 0;
