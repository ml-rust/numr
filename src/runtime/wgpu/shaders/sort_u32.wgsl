...
@group(0) @binding(0) var<storage, read_write> sort_input: array<u32>;
@group(0) @binding(1) var<storage, read_write> sort_output: array<u32>;
...
fn sort_u32(
    ...
    for (var i = tid; i < n; i = i + WORKGROUP_SIZE) {
        if (i < sort_size) {
            ...
            shared_vals[i] = select(4294967295u, 0u, descending);
            shared_idxs[i] = i32(i);
        }
    }
    workgroupBarrier();
