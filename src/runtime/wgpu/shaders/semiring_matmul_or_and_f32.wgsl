// Semiring matmul: or_and for f32
// C[i,j] = OR_k( A[i,k] AND B[k,j] )  (logical, mapped to float 0.0/1.0)
// Entry points: semiring_matmul_or_and_f32, batched_semiring_matmul_or_and_f32
...
@compute @workgroup_size(16, 16, 1)
fn batched_semiring_matmul_or_and_f32(
    @builtin(global_invocation_id) global_id: vec3<u32>
) {
    let M = sr_params.M;
    let K = sr_params.K;
