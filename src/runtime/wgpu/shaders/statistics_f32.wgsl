// Statistics shaders - F32
// mode_dim_f32: Find most frequent value in sorted data along reduce dimension
...
@group(0) @binding(0) var<storage, read_write> sorted: array<f32>;
@group(0) @binding(1) var<storage, read_write> mode_values: array<f32>;
@group(0) @binding(2) var<storage, read_write> mode_counts: array<i32>;
@group(0) @binding(3) var<uniform> params: ModeParams;

@compute @workgroup_size(1)
fn mode_dim_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let out_idx = gid.x;
