// Chi-squared distribution sampling for f32
...
fn chi_squared_f32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    if idx < params.numel {
        var state = pcg_init(params.seed, idx);
        // Chi-squared(df) = Gamma(df/2, 2)
        out[idx] = f32(sample_gamma_mt(&state, params.df / 2.0, 2.0));
    }
}
