// Auto-generated unique_with_counts operations for u32
...
@group(0) @binding(0) var<storage, read> sorted_input: array<u32>;
...
fn mark_boundaries_u32(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let idx = global_id.x;
...
@group(0) @binding(0) var<storage, read> scatter_sorted: array<u32>;
@group(0) @binding(1) var<storage, read> prefix_sum: array<u32>;
@group(0) @binding(2) var<storage, read_write> unique_values: array<u32>;
...
@compute @workgroup_size(256)
fn scatter_unique_with_counts_u32(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let idx = global_id.x;
