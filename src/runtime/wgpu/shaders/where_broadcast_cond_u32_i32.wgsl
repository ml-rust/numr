// where_broadcast_cond: condition=u32, output=i32
// out[i] = cond[cond_offset] != 0 ? x[x_offset] : y[y_offset] (with broadcasting)
...
@group(0) @binding(0) var<storage, read_write> bc_cond: array<u32>;
@group(0) @binding(1) var<storage, read_write> bc_x: array<i32>;
@group(0) @binding(2) var<storage, read_write> bc_y: array<i32>;
@group(0) @binding(3) var<storage, read_write> bc_out: array<i32>;
...
fn where_broadcast_cond_u32_i32(@builtin(global_invocation_id) gid: vec3<u32>) {
    let idx = gid.x;
    ...
    let cond_val = bc_cond[cond_offset] != 0u;
