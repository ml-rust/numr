...
use crate::runtime::cuda::kernels::{
    ScatterReduceOpCuda, launch_argmax_dim, launch_argmin_dim, launch_bincount_weighted,
    launch_copy, launch_embedding_lookup, launch_fill_with_f64, launch_gather, launch_gather_nd,
    launch_index_put, launch_index_select, launch_masked_count, launch_masked_fill,
    launch_masked_prefix_sum, launch_masked_select, launch_scatter, launch_scatter_reduce,
...
impl IndexingOps<CudaRuntime> for CudaClient {
    ...
    fn gather_nd(
        ...
        let shape_ptr = CudaRuntime::allocate(shape_bytes, &self.device);
        let strides_ptr = CudaRuntime::allocate(shape_bytes, &self.device);
        ...
        // Clean up temporary device allocations
        CudaRuntime::deallocate(shape_ptr, shape_bytes, &self.device);
        CudaRuntime::deallocate(strides_ptr, shape_bytes, &self.device);
    ...
    fn bincount(
        ...
        let out = Tensor::<CudaRuntime>::empty(&[output_len], out_dtype, &self.device);
