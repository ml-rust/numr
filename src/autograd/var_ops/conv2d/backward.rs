//! Backward pass for differentiable 2D convolution.
...
use super::grad_math::{conv2d_input_backward, conv2d_weight_backward};
    ...
    R::Client: ConvOps<R> + TensorOps<R> + ReduceOps<R> + BinaryOps<R> + ScalarOps<R>,
{
    fn backward(
        ...
        let d_input = if needed[0] {
            Some(conv2d_input_backward::<R, _>(
                &client,
                grad_output,
                &self.saved_weight,
                &self.input_shape,
        ...
        let d_weight = if needed[1] {
            Some(conv2d_weight_backward::<R, _>(
                &client,
                grad_output,
                &self.saved_input,
